...
impl DbInterface {
    ...
    pub fn query_account_to_customer_from_mail(&self, mail: &String) -> String {
        let sql = String::from("select account_id from account a inner join customer c on a.customer_id == c.customer_id where c.email = ?1;");
        self.query_account_to_customer(&sql, mail)
    }

    pub fn query_account_to_customer(&self, sql: &String, arg: &String) -> String {
        let mut stmt = self.con.prepare(sql).unwrap();
    ...
    pub fn query_daily_closing(&self, account_id: &String) -> (i32, String, i32, String) {
        let sql = String::from("select * from daily_closing where account_id = ?1 order by date desc;");
        let mut stmt = self.con.prepare(&sql).unwrap();
        stmt.query_one([account_id], |row| {
    ...
    pub fn create_daily_closing(&self, account_id: &String, new_balance: i32) {
        let sql = String::from("insert into daily_closing values(NULL, ?1, ?2,(select date('now', 'localtime')))");
    ...
    pub fn update_daily_closing(&self, closing_id: i32, new_balance: i32) {
        let sql = String::from("update daily_closing set balance = ?2 where closing_id = ?1;");
    ...
    pub fn create_transfer(&self, transfer_type: i32, account_id_sender: &String, account_id_receiver: &String, amount: i32, reference: &String) {
        let sql = String::from("insert into transfer values(NULL, ?1, ?2, ?3, ?4, (select datetime('now', 'localtime')), ?5);");
